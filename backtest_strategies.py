...
for ticker in tech_stocks:
    try:
        ...
        # Check if we have a MultiIndex in the columns and flatten if needed
        if isinstance(df.columns, pd.MultiIndex):
            # Extract first level values (column names) and keep as new column names
...
# Display more information about DataFrame structure for just the first ticker
first_ticker = tech_stocks[0]
if first_ticker in stock_data:
    first_df = stock_data[first_ticker]
    print(f"\nDebug info for {first_ticker}:")
    print(f"DataFrame shape: {first_df.shape}")
    print(f"DataFrame index type: {type(first_df.index)}")
    print(f"DataFrame columns: {first_df.columns.tolist()}")
    print(f"Close column type: {type(first_df['Close'])}")
    print(f"First 3 rows of Close:")
    for i in range(min(3, len(first_df))):
        print(f"  Row {i}: {first_df['Close'].iloc[i]} ({type(first_df['Close'].iloc[i])})")
...
def implement_strategies(stock_data):
    ...
    for ticker, df in stock_data.items():
        ...
        for i in range(1, len(df)):
            ...
            # Calculate average volume locally
            if i >= 50:
