...
def visualize_results(backtest_results, performance, stock_to_sector):
    ...
    all_swing_perf = [(ticker, perf['swing_trade']['total_return']) 
                      for ticker, perf in performance.items() 
                      if 'swing_trade' in perf]
    all_breakout_perf = [(ticker, perf['breakout']['total_return']) 
                         for ticker, perf in performance.items() 
                         if 'breakout' in perf]
    ...
    tickers = [t[0] for t in swing_top3 + breakout_top3]
    values = [t[1] for t in swing_top3] + [t[1] for t in breakout_top3]
    strategy_labels = ['Swing Trade'] * 3 + ['Breakout'] * 3
    bar_colors = ['blue'] * 3 + ['orange'] * 3
    bar_labels = [f"{t[0]} ({stock_to_sector[t[0]]})" for t in swing_top3 + breakout_top3]
    
    axes[1].bar(bar_labels, values, color=bar_colors)
    ...
    # 3. Create summary visualization comparing sectors
    ...
    swing_returns = [sector_metrics[sector]['swing_return'] for sector in sectors_list]
    breakout_returns = [sector_metrics[sector]['breakout_return'] for sector in sectors_list]
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(sectors_list)
    ax.set_ylabel('Total Return (%)')
    ax.legend()
    ax.grid(axis='y', linestyle='--', alpha=0.7)
